...
def filter_matrices(city, data, tracts,type):
    tracts['tract_id'] = pd.to_numeric(tracts['tract_id'])

    for i in range(len(tracts)):
        tract = tracts['tract_id'][i]
        temp = data[data['destination']==tract]
        temp.to_csv(f".../mobility/data/transit matrices/{city}/{type}/{tract}.csv", index=False)
    return  data
...
def download_matrices():
    ...
    for name, url in transit_urls.items():
        ...
        chi_od = filter_matrices("CHICAGO", data, chi_tracts,type)
        nyc_od = filter_matrices("NYC", data, nyc_tracts, type)
