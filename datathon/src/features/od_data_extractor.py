...
class DataExtractor:
    ...
    def load_all_data(self) -> None:
        ...
        for index, date in enumerate(self.dates):
            self.all_data[date] = np.loadtxt(f"{self.data_path}/{date}.csv",dtype=np.uint, delimiter=",")
    ...
    def get_trips_to_d(self) -> pd.core.series:
        ...
        for date in self.dates:
            trips_by_date[date] = self.all_data[date][d_index]
        return trips_by_date
    
    def get_trips_df(self) -> pd.core.frame.DataFrame:
        trips_data = self.get_trips_to_d()
        all_data = []
        for date, data in trips_data.items():
               
            date_data = pd.DataFrame(
                zip(
                    self.tract_ids, 
                    [self.destination for i in data],
                    data, 
                    [date for i in data]
                )
            )
    ...
    def get_timeseries_df_multi(self) -> pd.core.frame.DataFrame:
        ...
        for nested_idx, geoid in enumerate(self.destinations):
            try:
                ...
                df = pd.concat([df, self.get_trips_df()], ignore_index=True )
            except:
                print(f"issue with destination {self.destination}")
        self.set_df(df)
        return df
